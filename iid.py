...
def train(sampler, input_shape, config, device="cuda", verbose=0):
    ...
    for epoch in range(1, config["epochs"] + 1):  # Epoch/step counts will be 1-based.
        # Example: if we want to zap once every 3 epochs, then zap at beginning of 4th epoch (epoch - 1).
        if lobo_rate and lobo_size and epoch != 1 and (epoch - 1) % lobo_rate == 0:
            ...
            log.info(f"Zapping {num_zapped} classes.")
