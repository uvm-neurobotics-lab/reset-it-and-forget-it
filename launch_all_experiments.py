...
models_84px = [
    ...
    {"model_name": "vgg16_bn", "encoder": "vgg16_bn_encoder", "encoder_args": {}},
]

# Optimal LRs are different depending on train_method. But we can sweep over various rates, regardless of model.
iid_opt_cfg = [
    {"epochs": 30, "batch_size": 128, "optimizer": "Adam", "lr": 0.003},
    # {"epochs": 30, "batch_size": 128, "optimizer": "Adam", "lr": 0.001},
    {"epochs": 30, "batch_size": 128, "optimizer": "Adam", "lr": 0.0003},
    {"epochs": 30, "batch_size": 128, "optimizer": "SGD",
     "optimizer_args": {"lr": 0.1, "momentum": 0.9, "weight_decay": 0.0001},
     "lr_scheduler": "CosineAnnealingLR", "lr_scheduler_args": {"T_max": 30}},
    {"epochs": 30, "batch_size": 256, "optimizer": "Adam", "lr": 0.003},
    # {"epochs": 30, "batch_size": 256, "optimizer": "Adam", "lr": 0.001},
    {"epochs": 30, "batch_size": 256, "optimizer": "Adam", "lr": 0.0003},
    {"epochs": 30, "batch_size": 256, "optimizer": "SGD",
     "optimizer_args": {"lr": 0.1, "momentum": 0.9, "weight_decay": 0.0001},
     "lr_scheduler": "CosineAnnealingLR", "lr_scheduler_args": {"T_max": 30}},
]
inner_outer_opt_cfg = [
    {"inner_lr": 0.1, "outer_lr": 0.001},  # Only good for Meta, not so good for ASB.
    {"inner_lr": 0.01, "outer_lr": 0.001},
    {"inner_lr": 0.001, "outer_lr": 0.001},
    {"inner_lr": 0.001, "outer_lr": 0.0001},
    # {"inner_lr": 0.0001, "outer_lr": 0.001},  # Only good for ASB, not so good for Meta.
    {"inner_lr": 0.0001, "outer_lr": 0.0001},
]
...
iid_lobo_options = [
    ...
    {"lobo_rate": 5},
    {"lobo_rate": 10},
    {"lobo_rate": 1, "lobo_frac": 0.1},
    {"lobo_rate": 3, "lobo_frac": 0.1},
...
def launch_jobs(parser, args, launcher_args):
    ...
    total *= (num_iid * len(iid_lobo_options) * len(iid_opt_cfg) +
              num_inner_outer * len(lobo_options) * len(inner_outer_opt_cfg))
        ...
        for method in train_method:
            ...
            for model_desc in models:
                ...
                for lobo in lobopts:
                    ...
                    # In order to support fractions of the total number of classes we need to convert those fractions
                    # to sizes here. Note how this is a hack because it depends on the classifier config to tell us how
                    # many total classes there are.
                    if "lobo_frac" in lobo:
                        lobo = copy(lobo)
                        lobo["lobo_size"] = round(lobo["lobo_frac"] *
                                                  lbcfg["model_args"]["classifier_args"]["num_classes"])
                        del lobo["lobo_frac"]
                    lbcfg.update(lobo)

                    # LR settings depend on train method.
                    opt_cfgs = iid_opt_cfg if method == "-iid" else inner_outer_opt_cfg
